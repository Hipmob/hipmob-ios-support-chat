...
/**
 * Set to YES to have the controller only show the chat button if an operator is present. By default the chat button is always shown (defaults to NO). The user id passed when the search view was initialized must NOT be nil for the chat button to appear.
 */
@property (nonatomic, assign) BOOL showChatOnlyIfOperatorAvailable;
